...
│   ├── Get-ClusterSnapshot.ps1         # inventory/runs/capacity/GC/alerts
│   └── ConvertTo-DashboardModel.ps1     # totals, warnings, stale-data merge
└── output/                              # created locally; Git ignored
```
...
Run `Run-CohesityDashboard.ps1`. If `config.psd1` is absent, the launcher creates
it from `config.example.psd1`. The included defaults already use:
...
2. Open alerts are retrieved directly from each cluster with `accessClusterId`.
3. Object inventory and unresolved failures use recent object-level run details for all six workloads.
4. SQL and Oracle Full, Incremental, and Log streams are evaluated independently.
...
- If Cohesity marks a PG failed but returns no object details, the dashboard reports a collection warning instead of inventing a PG-level object failure.
...
| Workload count is zero unexpectedly | Read the on-screen collection warning; inventory is derived from `includeObjectDetails=true` run results |
