...
The leading number and trailing date can change. Match only these stable fragments:
...
### 3. Add the Condition — recommended expression method

Delete the existing two-row condition if it keeps returning False or creates unwanted blank rows.

1. Select **+ New step**.
2. Add **Control → Condition**.
3. In the left **Choose a value** field, select **Expression**.
4. Paste this exact expression without adding `@`:

```text
or(contains(toLower(coalesce(triggerOutputs()?['body/subject'],'')),'custer common gflag report'),contains(toLower(coalesce(triggerOutputs()?['body/subject'],'')),'custer specific gflag report'))
```

5. Set the operator to:

```text
is equal to
```

6. In the right **Choose a value** field, select **Expression** and enter:

```text
true
```

The completed condition is logically:
...
This method is case-insensitive and avoids the extra-row behaviour of the visual OR condition builder.
...
Click inside **File Name**, open **fx / Expression**, and paste:

```text
concat(if(contains(toLower(coalesce(triggerOutputs()?['body/subject'],'')),'custer common gflag report'),'Cohesity_Common_GFlags_','Cohesity_Custer_Specific_GFlags_'),formatDateTime(utcNow(),'yyyy-MM-dd_HHmmssfff'),'.html')
```
...
The expression is case-insensitive. The timestamp is UTC and includes milliseconds.

### 7. Configure File Content

1. Click inside **File Content**.
2. Open **Dynamic content**.
3. Select **Body** from the Outlook trigger.
...
When a new email arrives in the selected Outlook folder
                    ↓
Expression detects Custer Common OR Custer Specific?
          ┌─────────┴─────────┐
         True                False
          ↓                    ↓
...
3. Send, receive, or forward a new Custer Common or Custer Specific report into the monitored Outlook folder.
4. Open **Run history**.
5. Open the trigger and confirm its **Subject** output contains the expected text.
6. Confirm the Condition followed the **True** branch.
7. Confirm the HTML file was created in `Documents/Cohesity GFlag Reports`.
8. Open the file and verify that the complete email body and tables are present.
...
## Troubleshooting a False condition

Open **Run history → the run → Condition → Inputs**.

- The left-side evaluated value should be `true`.
- If it is `false`, open the Outlook trigger's outputs and inspect the exact `subject` value.
- If `subject` is blank, the condition is referencing the wrong trigger or dynamic-content source.
- Do not use `Cluster`; the observed report subjects use `Custer`.
