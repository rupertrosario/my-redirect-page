...
export default async function () {
  ...
  const TRIGGER_CODES = [
    ...
    // Add new interface-related alert codes here.
    // "NEWCODE1",
    // "NEWCODE2",
    // "CE_FULL_NEWCODE"
  ];
  ...
  function toArray(v) {
    ...
    if (Array.isArray(v?.result?.results)) {
      return v.result.results;
    }
  ...
  function isTrigger(code) {
    ...
    for (const trigger of TRIGGER_CODES) {
  ...
  try {
    ...
    upstream =
      await wfResult(PREDECESSOR_TASK);
    ...
    return {
      ok: false,
      error:
        `Cannot read result("${PREDECESSOR_TASK}")`,
      teamIncidents: [],
      dcIncidents: []
    };
  }
  ...
  const alertRows =
    toArray(upstream?.results ?? upstream);
