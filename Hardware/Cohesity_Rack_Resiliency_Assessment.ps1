...
# ABSOLUTE RULE: HTTP GET ONLY. No configuration changes are performed.
...
function Invoke-CohesityGet {
    ...
    # Mandatory safety gate before EVERY request.
    $method = "GET"
    if ($method -ne "GET") {
...
function Get-PropertyValue {
    param(
        $Object,
        [string[]]$Names,
        $Default = "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"
    )
...
function Normalize-Objects {
    param($Response,[string[]]$WrapperNames,[string[]]$IdentityProperties)
    ...
    foreach ($identity in $IdentityProperties) {
        if ($Response.PSObject.Properties[$identity]) { return @($Response) }
    }
...
function To-Text {
    param($Value)
    if ($null -eq $Value) { return "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }
    if ($Value -is [System.Array]) { return (@($Value) -join "; ") }
    $text = ([string]$Value).Trim()
    if ([string]::IsNullOrWhiteSpace($text)) { return "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }
    return $text
}
...
function Convert-ToCapacityText {
    ...
    if ($null -eq $n) { return (To-Text $Value) }
    if ($n -ge 1PB) { return ("{0:N2} PB" -f ($n / 1PB)) }
...
function Get-AssessmentFlag {
    ...
    if ($InsufficientDomains -or $WarningOrSuboptimal) { return "WARNING" }
    if ($NoRacks -or $UnassignedChassis -or $UnevenDistribution -or $MixedArchitecture) { return "REVIEW" }
    return "NORMAL"
}
...
# -----------------------------------------------------------------------------
# 1. CLUSTER DISCOVERY / CLUSTER DATA
# -----------------------------------------------------------------------------
...
try {
    ...
    $clusterObjects = @(Normalize-Objects $clusterResponse @("clusters","items") @("id","clusterId"))
}
catch {
...
foreach ($cluster in $clusterObjects) {
    ...
    if ($clusterId -eq "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION") { continue }
...
foreach ($clusterRow in $clusterRows) {
    ...
    try {
        ...
        $nodes = @(Normalize-Objects $nodeResponse @("nodes","items","nodeList") @("nodeId","id"))
    }
    catch {
    ...
    # ---------------------- NODE HARDWARE --------------------
    # Supplementary GET. Do not rely on it for node count because some versions
    # return a cluster/chassis scoped structure rather than all node records.
    try {
        ...
        $hardwareObjects = @(Normalize-Objects $hardwareResponse @("hardwareInfo","hardwareInfos","items") @("nodeModel","productModel","chassisModel"))

        # If node identifiers are available, enrich node rows. No serials are retained.
        foreach ($hardware in $hardwareObjects) {
            $hardwareNodeId = To-Text (Get-PropertyValue $hardware @("nodeId","id"))
            if ($hardwareNodeId -eq "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION") { continue }

            $target = $nodeRows | Where-Object {
                $_.ClusterAlias -eq $clusterAlias -and $_.NodeId -eq $hardwareNodeId
            } | Select-Object -First 1
        ...
        # Supplementary only: do not fail the cluster assessment.
    }
    ...
    try {
        ...
        $chassisList = @(Normalize-Objects $chassisResponse @("chassis","items","chassisList") @("id","chassisId","nodeIds"))
    }
    catch {
    ...
    foreach ($chassis in $chassisList) {
        ...
        foreach ($nodeId in $nodeIds) {
            $targetNode = $nodeRows | Where-Object {
                $_.ClusterAlias -eq $clusterAlias -and $_.NodeId -eq [string]$nodeId
            } | Select-Object -First 1
            if ($targetNode) {
                $targetNode.ChassisId = $chassisId
                if ($targetNode.HardwareModel -eq "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION") {
    ...
    try {
        ...
        $racks = @(Normalize-Objects $rackResponse @("racks","items","rackList") @("id","rackId"))
    }
    catch {
    ...
    foreach ($rack in $racks) {
        ...
        if ($rackChassisIds.Count -eq 0 -and $rackId -ne "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION") {
            $rackChassisIds = @($chassisRows |
                Where-Object { $_.ClusterAlias -eq $clusterAlias -and $_.RackId -eq $rackId } |
                Select-Object -ExpandProperty ChassisId)
        }

        $rackNodeIds = @()
        foreach ($cid in $rackChassisIds) {
            $rackNodeIds += @($chassisRows |
                Where-Object { $_.ClusterAlias -eq $clusterAlias -and $_.ChassisId -eq [string]$cid } |
                ForEach-Object { if ($_.NodeIds) { $_.NodeIds -split ';' } })
        }
        $rackNodeIds = @($rackNodeIds | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } | Select-Object -Unique)
    ...
    try {
        ...
        $storageDomains = @(Normalize-Objects $sdResponse @("storageDomains","items") @("id","storageDomainId"))
    }
    catch {
    ...
    foreach ($sd in $storageDomains) {
        ...
        $ec = To-Text (Get-PropertyValue $sd @("ecConfig","erasureCodingConfig","ecConfiguration"))
        $rf = To-Text (Get-PropertyValue $sd @("replicationFactor","rf"))
        $ft = To-Text (Get-PropertyValue $sd @("faultTolerance","faultToleranceLevel","failureDomain"))
        ...
        if ($sdId -eq "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION") {
            $unavailableFields.Add("Storage Domain ID") | Out-Null
            continue
        }

        # ---------------- FT OPTIONS FOR EVERY STORAGE DOMAIN ----------------
        try {
            ...
            $globalLevel = if ($null -ne $globalTolerance) { To-Text (Get-PropertyValue $globalTolerance @("faultToleranceLevel","level")) } else { "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }
            $globalCount = if ($null -ne $globalTolerance) { To-Text (Get-PropertyValue $globalTolerance @("count")) } else { "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }
            ...
            $options = @(Normalize-Objects $ftResponse @("options","faultToleranceOptions","availableOptions") @("ecConfig","replicationFactor","rf"))

            if ($options.Count -eq 0) {
                $ftOptionRows += [pscustomobject][ordered]@{
                    ClusterAlias=$clusterAlias; ClusterId=$clusterId; StorageDomain=$sdAlias; StorageDomainId=$sdId
                    Enabled=$enabled; GlobalFTLevel=$globalLevel; GlobalFTCount=$globalCount; FailureDomainCount=$failureDomainCount
                    DefaultFaultTolerance=$defaultFt; DefaultEC=$defaultEc; DefaultRF=$defaultRf
                    DiskFailuresTolerated="NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"
                    FailureDomainFailuresTolerated="NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"
                    EC="NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"; RF="NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"
                    Disabled="NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"; HasWarning="NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"
                    IsSuboptimal="NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"; MinFailureDomainsRequired="NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"
                    MinFailureDomainsToHeal="NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"; InlineECSupport="NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"
                }
            }
            else {
    ...
    $clusterSds = @($storageDomainRows | Where-Object ClusterAlias -eq $clusterAlias)
    $clusterFt = @($ftOptionRows | Where-Object ClusterAlias -eq $clusterAlias)

    $unassignedChassis = @($clusterChassis | Where-Object {
        $_.RackId -eq "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"
    }).Count

    $rackNodeCounts = @($clusterRacks | Select-Object -ExpandProperty NodeCount)
    $largestRackNodes = if ($rackNodeCounts.Count -gt 0) { ($rackNodeCounts | Measure-Object -Maximum).Maximum } else { 0 }
    $largestRackPct = if ($clusterNodes.Count -gt 0 -and $largestRackNodes -gt 0) {
        [math]::Round(($largestRackNodes / $clusterNodes.Count) * 100, 1)
    }
    else { 0 }
    ...
    if ($rackNodeCounts.Count -gt 1) {
        ...
        if ($maxNodes -ne $minNodes) { $unevenDistribution = $true }
    }

    $nodesPerChassis = @($clusterChassis | Select-Object -ExpandProperty NodeCount -Unique)
    $mixedArchitecture = ($nodesPerChassis.Count -gt 1 -and ($nodesPerChassis -contains 1) -and ($nodesPerChassis | Where-Object { $_ -ge 4 }).Count -gt 0)

    $warningOrSuboptimal = @($clusterFt | Where-Object {
        ([string]$_.HasWarning -match '^(?i:true|yes|1)$') -or
        ([string]$_.IsSuboptimal -match '^(?i:true|yes|1)$') -or
        ([string]$_.Disabled -match '^(?i:true|yes|1)$')
    }).Count -gt 0

    $insufficientDomains = $false
    foreach ($option in $clusterFt) {
        $minReq = Get-NumericValue $option.MinFailureDomainsRequired
        if ($null -ne $minReq -and $clusterRacks.Count -lt $minReq) {
            $insufficientDomains = $true
            break
        }
    }

    $failureDomainLevels = @($clusterFt | Select-Object -ExpandProperty GlobalFTLevel -Unique | Where-Object {
        $_ -ne "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"
    })
    $currentFailureDomain = if ($failureDomainLevels.Count -eq 1) { $failureDomainLevels[0] } elseif ($failureDomainLevels.Count -gt 1) { ($failureDomainLevels -join "; ") } else { $clusterRow.FailureDomain }

    $failuresToleratedValues = @($clusterFt | Select-Object -ExpandProperty GlobalFTCount -Unique | Where-Object {
        $_ -ne "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"
    })
    $failuresTolerated = if ($failuresToleratedValues.Count -gt 0) { $failuresToleratedValues -join "; " } else { "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }

    $failureDomainCountValues = @($clusterFt | Select-Object -ExpandProperty FailureDomainCount -Unique | Where-Object {
        $_ -ne "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION"
    })
    $failureDomainCount = if ($failureDomainCountValues.Count -gt 0) { $failureDomainCountValues -join "; " } else { "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }

    $hardwareMix = @($clusterNodes | Group-Object HardwareBucket | Sort-Object Name | ForEach-Object { "$($_.Name)=$($_.Count)" }) -join "; "
    if ([string]::IsNullOrWhiteSpace($hardwareMix)) { $hardwareMix = "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }
...
# Internal supporting CSVs contain IDs needed for local correlation, but no names/IPs/serials.
$nodeRows | Select-Object ClusterAlias,ClusterId,NodeId,ChassisId,HardwareModel,HardwareBucket,NodeModel,ProductModel,ProductModelType,SlotNumber,Status,Reachability,PhysicalCapacity |
    Export-Csv -Path $internalNodeCsv -NoTypeInformation -Encoding UTF8
$chassisRows | Select-Object ClusterAlias,ClusterId,ChassisId,HardwareModel,HardwareBucket,NodeCount,NodeIds,RackId,Location |
    Export-Csv -Path $internalChassisCsv -NoTypeInformation -Encoding UTF8
$rackRows | Select-Object ClusterAlias,ClusterId,RackAlias,RackId,ChassisCount,ChassisIds,NodeCount,NodeIds |
...
if ($null -eq $totalUnassignedChassis) { $totalUnassignedChassis = 0 }
...
$sanitized.Add("|---|---:|---:|---:|---|---|---|---|---|")
foreach ($row in $internalClusterSummary) {
    $rackDist = @($rackRows | Where-Object ClusterAlias -eq $row.ClusterAlias | ForEach-Object { "$($_.RackAlias)=$($_.NodeCount) nodes" }) -join "; "
    if ([string]::IsNullOrWhiteSpace($rackDist)) { $rackDist = "No configured racks returned" }
...
$sanitized.Add("|---|---|---|---|---|---|---|---|---|")
foreach ($sd in $storageDomainRows) {
    $options = @($ftOptionRows | Where-Object { $_.ClusterAlias -eq $sd.ClusterAlias -and $_.StorageDomain -eq $sd.StorageDomain })
    $fdAvailable = @($options | Select-Object -ExpandProperty FailureDomainCount -Unique) -join "; "
    if ([string]::IsNullOrWhiteSpace($fdAvailable)) { $fdAvailable = "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }

    $rackOptions = @($options | Where-Object {
        ([string]$_.GlobalFTLevel -match '(?i)rack') -or
        ([string]$_.FailureDomainFailuresTolerated -notmatch '^NOT AVAILABLE')
    })
    $rackAvailable = if ($rackOptions.Count -gt 0) { "Yes" } else { "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }

    $minReq = @($options | Select-Object -ExpandProperty MinFailureDomainsRequired -Unique | Where-Object { $_ -ne "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }) -join "; "
    if ([string]::IsNullOrWhiteSpace($minReq)) { $minReq = "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }
    $minHeal = @($options | Select-Object -ExpandProperty MinFailureDomainsToHeal -Unique | Where-Object { $_ -ne "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }) -join "; "
    if ([string]::IsNullOrWhiteSpace($minHeal)) { $minHeal = "NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION" }

    $warn = @($options | Where-Object {
        ([string]$_.Disabled -match '^(?i:true|yes|1)$') -or
        ([string]$_.HasWarning -match '^(?i:true|yes|1)$') -or
        ([string]$_.IsSuboptimal -match '^(?i:true|yes|1)$')
    }).Count -gt 0

    $currentEcRf = "EC=$(Escape-Markdown $sd.CurrentEC); RF=$(Escape-Markdown $sd.CurrentRF)"
    $sanitized.Add("| $($sd.ClusterAlias) | $($sd.StorageDomain) | $currentEcRf | $(Escape-Markdown $sd.CurrentFT) | $(Escape-Markdown $fdAvailable) | $rackAvailable | $(Escape-Markdown $minReq) | $(Escape-Markdown $minHeal) | $(if($warn){'Yes'}else{'No'}) |")
}
$sanitized.Add("")
...
foreach ($cluster in $internalClusterSummary) {
    ...
    foreach ($rack in $clusterRackRows) {
        $pct = if ($cluster.Nodes -gt 0) { [math]::Round(($rack.NodeCount / $cluster.Nodes) * 100,1) } else { 0 }
...
$sanitized.Add("")
foreach ($cluster in $internalClusterSummary) {
    if ($cluster.Racks -eq 0) { $sanitized.Add("- $($cluster.ClusterAlias): no rack configuration was returned by GET /v2/racks.") }
    if ($cluster.UnassignedChassis -gt 0) { $sanitized.Add("- $($cluster.ClusterAlias): $($cluster.UnassignedChassis) chassis have no rack assignment returned by GET data.") }
    if ([string]$cluster.CurrentFailureDomain -match '(?i)node|chassis|rack') { $sanitized.Add("- $($cluster.ClusterAlias): current failure-domain value returned: $($cluster.CurrentFailureDomain).") }
    if ($cluster.WarningOrSuboptimalFT) { $sanitized.Add("- $($cluster.ClusterAlias): at least one Storage Domain FT option is disabled, warning, or suboptimal.") }
}
$sanitized.Add("")
$sanitized.Add("**Calculated**")
$sanitized.Add("")
foreach ($cluster in $internalClusterSummary) {
    if ($cluster.UnevenRackDistribution) { $sanitized.Add("- $($cluster.ClusterAlias): node distribution across returned racks is uneven; largest rack concentration is $($cluster.LargestRackNodePct)%.") }
    if ($cluster.MixedArchitecture) { $sanitized.Add("- $($cluster.ClusterAlias): returned chassis data shows mixed 1-node and 4-or-more-node chassis/block architecture.") }
    if ($cluster.InsufficientDomains) { $sanitized.Add("- $($cluster.ClusterAlias): configured rack count is below at least one returned minFailureDomainsRequired value.") }
}
$sanitized.Add("")
...
$sanitized.Add("")
$exceptionRows = @($internalClusterSummary | Where-Object {
    $_.Racks -eq 0 -or $_.UnassignedChassis -gt 0 -or $_.UnevenRackDistribution -or
    $_.WarningOrSuboptimalFT -or $_.InsufficientDomains -or $_.MixedArchitecture -or $_.IncompleteData
})
...
$sanitized.Add("")
$uniqueApiCalls = @($apiCalls | ForEach-Object { ($_ -replace ' \[.*$','') } | Select-Object -Unique)
...
$sanitized.Add("- Fields unavailable: $(if($unavailableFields.Count -gt 0){@($unavailableFields) -join '; '}else{'None explicitly tracked; individual unavailable values are marked NOT AVAILABLE THROUGH APPROVED READ-ONLY COLLECTION'})")
...
Write-Host "`n================ READ-ONLY COLLECTION SUMMARY ================" -ForegroundColor Cyan
...
if ($apiFailures.Count -gt 0) {
    Write-Host "`nGET FAILURES" -ForegroundColor Yellow
    $apiFailures | ForEach-Object { Write-Host "- $_" }
}

Write-Host "`nREAD-ONLY VALIDATION: No Cohesity configuration was modified during this assessment." -ForegroundColor Green
