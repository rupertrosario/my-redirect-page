...
Pasted text.txt
Document
see if you can use this and update teh code for me to test it 


Pasted text(2).txt
Document
 try this page and see..


Pasted text(3).txt
Document

Good. Do not change any files while it is running.

That means the earlier Helios/API-key failure is not blocking at the same point now, or at least it has moved further.
...
Then check worknotes
Get-Content (Join-Path $latest.FullName "worknotes_summary.txt")
Important
Do not push yet. First confirm these three things:

1. Script completed without terminating error
2. collection_warnings.txt is clean or understandable
3. worknotes_summary.txt looks correct
If it fails again, copy the last 30 lines of the console output. If it completes, then the next step is to push only:

backup_failures/Get-CohesityBackupFailureWindowConsolidator.ps1

and you forgot to push the instructions..what do you need as a remidner

Worked for 34s
Pushed now.

File added
backup_failures/01_AFTER_CURRENT_RUN_COMPLETES.md
Commit
984729a11c784ca3da7aa5526252eab47637fedf
What reminder I should not need
For this backup-failure work, the standing rule should be:

Any instructions I give in chat must also be pushed into backup_failures/ as a clear .md file.
Do not leave operational steps only in chat.
Important right now
Do not pull while the script is still running.

After the run finishes, pull and open this:
...
notepad .\backup_failures\01_AFTER_CURRENT_RUN_COMPLETES.md
Verified the file exists and starts with: “AFTER CURRENT RUN COMPLETES - Backup Failure validation and push steps.”


i thnk you are doing clusterwise agian..we decided to do it environmentwise..what changed
...
Worked for 2m 17s
Checked and corrected the two scripts you pasted.
...
4bb58dd067268837b0b248981613ce0f83a70a59
Correction made:

Added strict error handling.
If the main collector fails, formatter will not run.
Wrapper now exits 1 on collector failure and exits 0 only after successful wrapper flow.
Verified in GitHub: wrapper now has $ErrorActionPreference = "Stop" and catches collector failure before formatter.
...
ec076aaec4989fc1716ef2e66b074ede0d9e6799
Corrections made:

Raw CSV backups are now idempotent — repeated formatting will not overwrite *_raw.csv.
Formatter now handles both newer simple statuses and older lifecycle statuses.
Verified in GitHub: raw backup only copies if the raw file does not already exist, and formatter now supports both status models.

Important finding
The Helios failure block you showed is not from the wrapper or formatter. It is from the old consolidator you uploaded/pasted:

$apiKey = Get-CohesityApiKey
$headers = @{ accept = "application/json"; apiKey = $apiKey }
try {
    $clusterJson = Invoke-HeliosGetJson -Uri "$BaseUrl/v2/mcm/cluster-mgmt/info" -Headers $headers
That old consolidator also says:

Write-Host "Processing clusters alphabetically."
So yes — that pasted consolidator is the older cluster-first version, not the environment-first one. 


Do not pull now if the current run is still running
After it finishes:
...
[scriptblock]::Create((Get-Content ".\backup_failures\Cohesity_Backup_Failure_INC_Status_Update.ps1" -Raw)) | Out-Null
[scriptblock]::Create((Get-Content ".\backup_failures\Format-CohesityBackupFailureReport.ps1" -Raw)) | Out-Null

"Wrapper and formatter syntax OK"
The remaining problem is the consolidator. The two pasted scripts are now corrected.


check the consolidatory

Worked for 1m 8s
Checked the consolidator. The consolidator you pasted is the wrong/older one. Do not use it.

Problems in the pasted consolidator
It is cluster-first, not environment-first:

Write-Host "Processing clusters alphabetically."
and the collection function loops like this:

foreach ($cluster in $clusterList) {
    ...
    foreach ($env in (Get-EnvironmentMap)) {
That is cluster → environment, which is the opposite of what we decided. 


It also includes RemoteAdapter, which we decided to exclude:

[pscustomobject]@{ Label="RemoteAdapter"; Filter="kRemoteAdapter"; ... }
It also uses the older API-key function:

$apiKey = Get-CohesityApiKey
and the exact Helios failure block you saw is in that pasted/old consolidator. 


It also has the bad retry model where failedAttempts makes the object look failed before respecting final success:

$hasFailure =
    ($attempts.Count -gt 0) -or
That is the same retry problem we were fixing. 


Correct consolidator to use
Use this file instead:

Get-CohesityBackupFailureWindowConsolidator.ps1

This corrected one has:

Environment-first loop
RemoteAdapter excluded
Get-CohesityApiKeySafe
Baseline 30 / incremental 15
Retry-aware Get-ObjectState
No cluster-first "Processing clusters alphabetically"
Replace your local consolidator
Run this after the current run finishes:

Copy-Item `
  "C:\Users\<YOUR_USER>\Downloads\Get-CohesityBackupFailureWindowConsolidator.ps1" `
...
"Processing clusters alphabetically","RemoteAdapter","Get-CohesityApiKey$","Retry-aware rule","foreach (`$EnvironmentSpec"
Expected:

Processing clusters alphabetically   = no match
RemoteAdapter                         = only comments/state saying excluded, not environment map
Get-CohesityApiKey                    = no match
Retry-aware rule                      = match
foreach ($EnvironmentSpec             = match
Plain answer
Your pasted consolidator is not the one we want. It is the old clusterwise version. Replace only the consolidator with the corrected file above. Keep the 130-line wrapper and formatter as they are.


give me the full code 

Worked for 47s
Here is the full corrected consolidator code file:

Get-CohesityBackupFailureWindowConsolidator.ps1

Text copy also available:

Get-CohesityBackupFailureWindowConsolidator_FULL_CODE.txt

This is the corrected 1005-line consolidator. Use it to replace only:
