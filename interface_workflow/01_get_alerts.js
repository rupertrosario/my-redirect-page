// Dynatrace JS action: Get Cohesity interface-related alerts
// Purpose: Fetch alerts 1077, 1105, 13023 from Helios and preserve Cluster Location for DC routing.
// Cohesity calls are GET only.

import { credentialVaultClient } from "@dynatrace-sdk/client-classic-environment-v2";

export default async function () {
  var baseUrl = "https://helios.cohesity.com";
  var vaultName = "Cohesity_API_Key";
  var vaultId = "credentials_vault-312312";

  var AlertTypes = ["1077", "1105", "13023"];
  var AlertStates = ["kOpen", "kNote"];
  var MaxAlerts = 200;
  var LookbackHours = 24;

  function norm(v) {
    if (v === null || v === undefined) return "";
    return String(v).trim();
  }

  function toArrayMaybe(v) {
    if (!v) return [];
    return Array.isArray(v) ? v : [v];
  }

  function buildQuery(params) {
    var usp = new URLSearchParams();
    for (var k in params) {
      if (!Object.prototype.hasOwnProperty.call(params, k)) continue;
      var v = params[k];
      if (v === undefined || v === null) continue;
      if (Array.isArray(v)) {
        for (var i = 0; i < v.length; i++) {
          var s = norm(v[i]);
          if (s) usp.append(k, s);
        }
      } else {
        var sv = norm(v);
        if (sv) usp.append(k, sv);
      }
    }
    return usp.toString();
  }

  async function getJson(url, headers) {
    var resp = await fetch(url, { method: "GET", headers: headers });
    if (!resp.ok) {
      var txt = "";
      try { txt = await resp.text(); } catch (e) {}
      throw new Error("GET " + url + " -> HTTP " + resp.status + " " + txt);
    }
    return resp.json();
  }

  async function getKeyByName(name) {
    var all = await credentialVaultClient.getCredentials();
    var creds = (all && all.credentials) ? all.credentials : [];
    for (var i = 0; i < creds.length; i++) {
      if (creds[i] && creds[i].name === name) {
        var detail = await credentialVaultClient.getCredentialsDetails({ id: creds[i].id });
        return (detail && (detail.token || detail.password)) || null;
      }
    }
    return null;
  }

  var apiKey = null;
  var authMode = "vault-name";
  try {
    apiKey = await getKeyByName(vaultName);
    if (!apiKey) throw new Error("vault name not found");
  } catch (e) {
    var d2 = await credentialVaultClient.getCredentialsDetails({ id: vaultId });
    apiKey = (d2 && (d2.token || d2.password)) || null;
    authMode = "vault-id";
  }
  if (!apiKey) throw new Error("No Helios API key available.");

  function getProp(plist, keys) {
    var pl = toArrayMaybe(plist);
    for (var k = 0; k < keys.length; k++) {
      var wanted = keys[k];
      for (var i = 0; i < pl.length; i++) {
        var p = pl[i] || {};
        if (norm(p.key) !== wanted) continue;
        var v = p.value;
        if (!v) v = p.values;
        if (Array.isArray(v)) return v.length ? norm(v[0]) : "";
        return norm(v);
      }
    }
    return "";
  }

  function locationToString(loc) {
    if (!loc) return "";
    if (typeof loc === "string") return norm(loc);
    var guess = loc.name || loc.location || loc.site || loc.city || loc.region || loc.displayName || "";
    guess = norm(guess);
    if (guess) return guess;
    try { return JSON.stringify(loc); } catch (e) { return ""; }
  }

  function usecsToEtString(usecs) {
    if (!usecs) return "";
    var ms = Number(usecs) / 1000;
    if (!Number.isFinite(ms)) return "";
    var dtf = new Intl.DateTimeFormat("en-US", {
      timeZone: "America/New_York",
      year: "numeric", month: "2-digit", day: "2-digit",
      hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false
    });
    var parts = dtf.formatToParts(new Date(ms));
    function pick(t) { for (var i = 0; i < parts.length; i++) if (parts[i].type === t) return parts[i].value; return ""; }
    return pick("year") + "-" + pick("month") + "-" + pick("day") + " " + pick("hour") + ":" + pick("minute") + ":" + pick("second");
  }

  function safeCell(v) {
    if (v === null || v === undefined) return "";
    return String(v).replace(/\|/g, " ");
  }

  var commonHeaders = { accept: "application/json", apiKey: apiKey };
  var clusterData = await getJson(baseUrl + "/v2/mcm/cluster-mgmt/info", commonHeaders);
  var clusters = toArrayMaybe(clusterData && clusterData.cohesityClusters);

  var clusterMeta = {};
  for (var c = 0; c < clusters.length; c++) {
    var cl = clusters[c] || {};
    var cid = norm(cl.clusterId);
    if (!cid) continue;
    clusterMeta[cid] = {
      ClusterName: norm(cl.clusterName),
      Location: locationToString(cl.location)
    };
  }

  var nowMs = Date.now();
  var startUsecs = (nowMs - (LookbackHours * 60 * 60 * 1000)) * 1000;
  var endUsecs = nowMs * 1000;

  var alertsUrl = baseUrl + "/v2/alerts?" + buildQuery({
    maxAlerts: MaxAlerts,
    alertTypes: AlertTypes,
    alertStates: AlertStates,
    startDateUsecs: String(startUsecs),
    endDateUsecs: String(endUsecs)
  });

  var raw = [];
  var alertsFetched = 0;

  for (var ci = 0; ci < clusters.length; ci++) {
    var clus = clusters[ci] || {};
    var clusterId = norm(clus.clusterId);
    if (!clusterId) continue;
    var meta = clusterMeta[clusterId] || {};
    var headers = { accept: "application/json", apiKey: apiKey, accessClusterId: clusterId };

    var json;
    try {
      json = await getJson(alertsUrl, headers);
    } catch (e) {
      console.log("Alerts fetch failed for " + (meta.ClusterName || clusterId) + ": " + norm(e.message || e));
      continue;
    }

    var alerts = toArrayMaybe(json && json.alerts);
    alertsFetched += alerts.length;

    for (var ai = 0; ai < alerts.length; ai++) {
      var alt = alerts[ai] || {};
      var latestUsecs = Number(alt.latestTimestampUsecs || 0);
      if (!latestUsecs || latestUsecs < startUsecs || latestUsecs > endUsecs) continue;

      var nodeId = getProp(alt.propertyList, ["node_id", "nodeId"]);
      var ip = getProp(alt.propertyList, ["node_ip", "ip", "ipAddress", "host_ip", "source_ip", "sourceIp", "remote_ip", "remoteIp"]);
      var alertCause = (alt.alertDocument && (alt.alertDocument.alertCause || (alt.alertDocument.alertSummary && alt.alertDocument.alertSummary.alertCause))) || "";

      raw.push({
        Location: meta.Location || "",
        DcLocation: meta.Location || "",
        ClusterName: meta.ClusterName || norm(clus.clusterName),
        ClusterId: clusterId,
        ClusterNodeId: nodeId,
        NodeID: nodeId,
        IP: ip,
        NodeIP: ip,
        AlertCode: norm(alt.alertCode),
        Severity: norm(alt.severity),
        AlertState: norm(alt.alertState),
        AlertCause: norm(alertCause),
        AlertId: norm(alt.id),
        LatestTimeET: usecsToEtString(latestUsecs),
        LatestUsecs: latestUsecs
      });
    }
  }

  var latestMap = {};
  for (var i = 0; i < raw.length; i++) {
    var r = raw[i];
    var key = r.ClusterId + "|" + (r.ClusterNodeId || r.NodeIP || r.AlertId);
    if (!latestMap[key] || r.LatestUsecs > latestMap[key].LatestUsecs) latestMap[key] = r;
  }
  var results = Object.keys(latestMap).map(function (k) { return latestMap[k]; });
  results.sort(function (a, b) { return (b.LatestUsecs || 0) - (a.LatestUsecs || 0); });

  function toMarkdownTable(rows) {
    if (!rows.length) return "No interface-related alerts found.";
    var cols = ["Location", "ClusterName", "ClusterId", "ClusterNodeId", "NodeIP", "AlertCode", "Severity", "AlertState", "LatestTimeET"];
    var header = "| " + cols.join(" | ") + " |";
    var sep = "| " + cols.map(function () { return "---"; }).join(" | ") + " |";
    var body = rows.map(function (r) { return "| " + cols.map(function (c) { return safeCell(r[c]); }).join(" | ") + " |"; });
    return [header, sep].concat(body).join("\n");
  }

  return {
    authMode: authMode,
    alertTypes: AlertTypes,
    alertsFetched: alertsFetched,
    resultsCount: results.length,
    results: results,
    alertTargets: results,
    markdownTable: toMarkdownTable(results)
  };
}
