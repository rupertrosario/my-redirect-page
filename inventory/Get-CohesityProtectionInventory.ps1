...
function NowET {
    try { $tz=[TimeZoneInfo]::FindSystemTimeZoneById("Eastern Standard Time"); return ([TimeZoneInfo]::ConvertTimeFromUtc((Get-Date).ToUniversalTime(),$tz)).ToString("yyyy-MM-dd HH:mm:ss") }
...
function JsonGet { param([string]$Uri,[hashtable]$Headers) $r=Invoke-WebRequest -Uri $Uri -Headers $Headers -Method Get -UseBasicParsing; if(!$r -or [string]::IsNullOrWhiteSpace($r.Content)){return $null}; return ($r.Content|ConvertFrom-Json) }
    ...
    foreach($u in @("$baseUrl/v2/data-protect/policies?maxResultCount=1000","$baseUrl/v2/data-protect/policies")){
        try{
            ...
            foreach($p in @(Arr $list|Where-Object{$_ -and $_ -isnot [string]})){
                $id=First @((Prop $p @("id","policyId")))
                $name=First @((Prop $p @("name","policyName","displayName")))
                if($id -and $name){$m[$id]=$name}
            }
            if($m.Count -gt 0){break}
        }catch{}
    }
    return $m
}
function PolicyId { param($pg) return First @((Prop $pg @("policyId")),(Nest $pg "policyInfo.id"),(Nest $pg "policy.id")) }
function PolicyName { param($pg,[hashtable]$map) $id=PolicyId $pg; if($id -and $map.ContainsKey($id)){return $map[$id]}; $n=First @((Nest $pg "policyInfo.name"),(Nest $pg "policy.name"),(Prop $pg @("policyName"))); if($n){return $n}; return "UNRESOLVED_POLICY_NAME" }
function Params { param($pg,[string[]]$names) foreach($n in $names){$v=Prop $pg @($n); if($null -ne $v){return $v}}; return $null }
function GetPGs { param([string]$EnvApi,[hashtable]$Headers) $u="$baseUrl/v2/data-protect/protection-groups?environments=$EnvApi&isDeleted=false&isActive=true&includeLastRunInfo=true&maxResultCount=1000"; $j=JsonGet $u $Headers; return @(Arr (Prop $j @("protectionGroups")) | Where-Object { $_ }) }
function ObjectsFromParams { param($p,[string]$envApi) if($envApi -eq "kPhysical"){ $pt=First @((Prop $p @("protectionType"))); if($pt -eq "kVolume"){return @(Arr (Nest $p "volumeProtectionTypeParams.objects")|Where-Object{$_})}; return @(Arr (Nest $p "fileProtectionTypeParams.objects")|Where-Object{$_}) }; return @(Arr (Prop $p @("objects"))|Where-Object{$_}) }
function RunInfo { param($pg) $lr=Prop $pg @("lastRun"); $li=Prop $lr @("localBackupInfo","localSnapshotInfo"); $s=First @((Prop $li @("status")),(Prop $lr @("status"))); $rt=First @((Prop $li @("runType")),(Prop $lr @("runType"))); $st=Et (First @((Prop $li @("startTimeUsecs")),(Prop $lr @("startTimeUsecs")))); $en=Et (First @((Prop $li @("endTimeUsecs")),(Prop $lr @("endTimeUsecs")))); $ok=""; $oks=""; if(Good $s){$ok=$en;$oks=$s}; return [pscustomobject]@{LastRunStatus=$s;LastRunType=$rt;LastRunStartET=$st;LastRunEndET=$en;LastSuccessfulBackupET=$ok;LastSuccessfulBackupStatus=$oks} }

$InventoryDateET = NowET
$clusterJson=JsonGet "$baseUrl/v2/mcm/cluster-mgmt/info" (Headers)
$clusters=@(Arr (Prop $clusterJson @("cohesityClusters")) | ForEach-Object { [pscustomobject]@{ClusterName=First @((Prop $_ @("clusterName")),(Prop $_ @("displayName")),(Prop $_ @("name"))); ClusterId=First @((Prop $_ @("clusterId")),(Prop $_ @("id"))) } } | Sort-Object ClusterName)
...
Write-Host "`nSelect one environment per run:" -ForegroundColor Cyan
Write-Host "[1] Physical" -ForegroundColor Yellow
Write-Host "[2] Hyper-V" -ForegroundColor Yellow
Write-Host "[3] Nutanix AHV" -ForegroundColor Yellow
Write-Host "[X] Exit" -ForegroundColor Yellow
while($true){$es=Read-Host "Select environment"; if($es -match '^(x|X|q|Q)$'){return}; if($es -eq "1"){$selectedEnvironments=@($EnvironmentMap|Where-Object{$_.ApiName -eq "kPhysical"});break}; if($es -eq "2"){$selectedEnvironments=@($EnvironmentMap|Where-Object{$_.ApiName -eq "kHyperV"});break}; if($es -eq "3"){$selectedEnvironments=@($EnvironmentMap|Where-Object{$_.ApiName -eq "kAcropolis"});break}; Write-Host "Invalid selection. Select 1, 2, 3, or X." -ForegroundColor Red}
...
foreach($c in $selectedClusters){
    ...
    foreach($e in $selectedEnvironments){
        ...
        try{$pgList=@(GetPGs $e.ApiName $h)}catch{$errors+=[pscustomobject]@{Cluster=$c.ClusterName;Environment=$e.DisplayName;Stage="Get-ProtectionGroups";Error=$_.Exception.Message};continue}
        foreach($pg in $pgList){
            try{
                ...
                $pgRows+=[pscustomobject]@{PGKey=$pgKey;InventoryDateET=$InventoryDateET;Cluster=$c.ClusterName;ClusterId=$c.ClusterId;Environment=$e.DisplayName;ProtectionGroup=$pgName;ProtectionGroupId=$pgId;PolicyName=$policyName;PolicyId=$policyId;IsActive=Prop $pg @("isActive");IsDeleted=Prop $pg @("isDeleted");IsPaused=Prop $pg @("isPaused");ObjectCount=@($objs).Count;LastRunStatus=$r.LastRunStatus;LastRunType=$r.LastRunType;LastRunStartET=$r.LastRunStartET;LastRunEndET=$r.LastRunEndET;LastSuccessfulBackupET=$r.LastSuccessfulBackupET;LastSuccessfulBackupStatus=$r.LastSuccessfulBackupStatus;LastSuccessfulBackupAgeHours="";BackupFreshnessBucket="";IsSuccessLast24h=$false;IsSuccessLast48h=$false;ProtectionType=$ptype;StorageDomain=First @((Prop $pg @("storageDomainName")),(Nest $pg "storageDomain.name"));SourceId=$sourceId;SourceName=$sourceName;GlobalIncludeDisks=$globalIncludeDisks;GlobalExcludeDisks=$globalExcludeDisks;GlobalExcludePathCount=0;ObjectExcludePathCount=0;HasGlobalExclusions=($globalExcludeDisks -ne "");HasObjectExclusions=$false}
                foreach($o in $objs){
                    $objectName=ObjName $o; $objectId=ObjId $o; $objectKey="$pgKey|$(First @($objectId,$objectName))"
                    $objectType=First @((Prop $o @("objectType","type","entityType"))); if(!$objectType){$objectType=if($e.ApiName -eq "kPhysical"){"PhysicalObject"}else{"VirtualMachine"}}
                    $objectIncludedDisks=Pack (Prop $o @("includeDisks")); $objectExcludedDisks=Pack (Prop $o @("excludeDisks"))
